...
VECTOR* PREFIX(new_init)(SIZE_T length, ...)
{
	...
	v = PREFIX(new)(length);
	va_start(ap, length);
	for (i = 0; i < length; i++) v->array[i] = va_arg(ap, VA_VALUE_T);
	va_end(ap);
...
VALUE_T PREFIX(sum)(VECTOR* v)
{
	SIZE_T i;
	VALUE_T res = 0;
	for (i = 0; i < v->length; i++) res += v->array[i];
	return res;
}
...
void PREFIX(mult)(VECTOR* dst, VALUE_T c, VECTOR* src)
{
	...
	for (i = 0; i < dst->length; i++) dst->array[i] = c * src->array[i];
}
...
void PREFIX(div)(VECTOR* dst, VECTOR* src, VALUE_T c)
{
	SIZE_T i;
	claim(dst->length == src->length);
	for (i = 0; i < dst->length; i++) dst->array[i] = src->array[i] / c;
}
