...
ivlist* all_perms(int n)
{
	...
	claim(n >= 0);
	dimvec = iv_new(n + 1);
	if (dimvec == NULL) return NULL;
	for (i = 0; i < iv_length(dimvec); i++) iv_elem(dimvec, i) = i;
...
ivector* string2perm(ivector* str)
{
	...
	N = 0;
	for (i = 0; i < n; i++)
		if (N < iv_elem(str, i)) N = iv_elem(str, i);
	N++;

	dimvec = iv_new_zero(N);
	if (dimvec == NULL) return NULL;
	for (i = 0; i < n; i++) iv_elem(dimvec, iv_elem(str, i))++;
	for (i = 1; i < N; i++) iv_elem(dimvec, i) += iv_elem(dimvec, i - 1);
...
ivector* str2dimvec(ivector* str)
{
	...
	{
		if (iv_elem(str, i) < 0) return NULL;
		if (n <= iv_elem(str, i)) n = iv_elem(str, i) + 1;
	}
	res = iv_new_zero(n);
	if (res == NULL) return NULL;
	for (i = 0; i < iv_length(str); i++) iv_elem(res, iv_elem(str, i))++;
	for (i = 1; i < n; i++) iv_elem(res, i) += iv_elem(res, i - 1);
	return res;
}
