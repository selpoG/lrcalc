...
{
	alloc_call_count++;
	if (alloc_call_count == alloc_trap_number) alloc_trap(NULL);
...
static void init_storage(void* p, size_t size)
{
	...
	*((size_t*)p) = size;
	...
	for (idx = sizeof(size_t); idx < ADD_TO_PTR; idx++) s[idx] = 0xa5;
	for (idx = size + ADD_TO_PTR; idx < size + ADD_TO_SIZE; idx++) s[idx] = 0xa5;
}
