...
static int _monk_add(int i, ivlincomb* slc, int rank, ivlincomb* res)
{
	...
	for (ivlc_first(slc, &itr); ivlc_good(&itr); ivlc_next(&itr))
	{
		...
		if (i <= n + 1)
		{
			...
			for (j = i - 1; j >= 1; j--)
				if (last < iv_elem(w, j - 1) && iv_elem(w, j - 1) < wi)
				{
					...
					u = iv_new(ulen);
					if (u == NULL) return -1;
					for (t = 0; t < n; t++) iv_elem(u, t) = iv_elem(w, t);
					for (t = n; t < ulen; t++) iv_elem(u, t) = t + 1;
					iv_elem(u, j - 1) = wi;
					iv_elem(u, i - 1) = last;
		...
		else
		{
			...
			if (u == NULL) return -1;
			for (t = 0; t < n; t++) iv_elem(u, t) = iv_elem(w, t);
			for (t = n; t < i - 2; t++) iv_elem(u, t) = t + 1;
			iv_elem(u, i - 2) = i;
			iv_elem(u, i - 1) = i - 1;
			if (ivlc_add_element(res, -c, u, iv_hash(u), LC_FREE_ZERO) != 0) return -1;
		}
...
static int _mult_ps(void** poly, int n, int maxvar, ivector* perm, int rank, ivlincomb* res);
...
ivlincomb* mult_poly_schubert(ivlincomb* poly, ivector* perm, int rank)
{
	...
	for (i = 0; i < n; i++) iv_free((ivector*)p[2 * i]);
	ml_free(p);
