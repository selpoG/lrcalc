...
void* alloc_heap_base = nullptr; /* Reference point in heap. */
...
void alloc_getenv()
{
	char* arg;
	alloc_reset();
	if ((arg = getenv("ML_FAIL_NUMBER"))) alloc_fail_number = atol(arg);
	if ((arg = getenv("ML_FAIL_SEGFAULT"))) alloc_fail_segfault = atol(arg);
	if ((arg = getenv("ML_MEMORY_PRINT"))) alloc_memory_print = atol(arg);
}

void alloc_report() { fprintf(stderr, "Memory balance: %ld\n", alloc_memory_used); }

void alloc_set_print(int prt) { alloc_memory_print = prt; }

void alloc_set_trap(size_t index) { alloc_trap_index = index; }
...
void alloc_set_trap_number(size_t N) { alloc_trap_number = N; }

void alloc_trap(const void* p)
{
...
static int alloc_check_fail()
{
	...
	if (alloc_fail_number == 0 || alloc_call_count < alloc_fail_number) return 0;
	if (alloc_fail_count++)
		fprintf(stderr, "malloc called after out-of-memory event.\n");
	else
		fprintf(stderr, "malloc provoke failure.\n");
	claim((alloc_fail_segfault == 0) || (alloc_fail_count < alloc_fail_segfault));
	return 1;
}
...
static void alloc_print(char c, const void* p, const char* name)
{
	...
	if (index == alloc_trap_index) alloc_trap(p);
}
...
static void init_storage(void* p, size_t size)
{
	...
	*((size_t*)p) = size;
	...
	for (size_t idx = sizeof(size_t); idx < ADD_TO_PTR; idx++) s[idx] = 0xa5;
	for (size_t idx = size + ADD_TO_PTR; idx < size + ADD_TO_SIZE; idx++) s[idx] = 0xa5;
}

static void check_storage(const void* p)
{
	size_t size = *((const size_t*)p);
	const unsigned char* s = p;
	for (size_t idx = sizeof(size_t); idx < ADD_TO_PTR; idx++)
		if (s[idx] != 0xa5)
		{
	...
	for (size_t idx = size + ADD_TO_PTR; idx < size + ADD_TO_SIZE; idx++)
		if (s[idx] != 0xa5)
		{
			fprintf(stderr, "WARNING: Pointer %zu (%p) dirty at index %zu.\n", p - alloc_heap_base, p, idx);
			claim(alloc_dirty_segfault == 0);
...
{
	void* p = alloc_check_fail() ? nullptr : malloc(size + ADD_TO_SIZE);
...
{
	void* p = alloc_check_fail() ? nullptr : calloc(1, size * num + ADD_TO_SIZE);
...
void* ml_realloc(void* p, size_t size)
{
	...
	p = alloc_check_fail() ? nullptr : realloc(p, size + ADD_TO_SIZE);
...
void alloc_test_oom(void(f)(void*), void* arg)
{
	...
	{
		fprintf(stderr, "alloc_test_oom: non-zero memory balance on first run.\n");
		return;
	}

	/* Total allocation calls by f(arg). */
	size_t alloc_count = alloc_call_count;
	fprintf(stderr, "alloc_test_oom: %zu alloc calls\n", alloc_count);

	for (size_t n = 1; n <= alloc_count; n++)
	{
		fprintf(stderr, "%zu ", n);
